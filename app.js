...
function normalizeChapterTitle(rawTitle) {
  const cleaned = rawTitle
    ...
    .replace(/[*_`~]/g, "")
    .trim();
  // 支持 "第1章：標題", "第1章 標題", "第1章·標題"
  const match = cleaned.match(/^(第[0-9零一二三四五六七八九十百千兩〇]+章)\s*[：:·\s]\s*(.+)$/);
  if (match) {
...
async function loadChapter(path) {
  ...
  try {
    ...
    setStatus(`第 ${activeIndex + 1} 章 / 共 ${files.length} 篇`);
...
function applyTheme(theme) {
  ...
  themeToggleButtons.forEach((button) => {
    button.textContent = theme === "dark" ? "切換成白天模式" : "切換成黑夜模式";
  });
  try {
